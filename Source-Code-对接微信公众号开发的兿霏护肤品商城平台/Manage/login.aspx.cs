namespace ZoomLaCMS.Manage
{
    ...
    public partial class login : System.Web.UI.Page
    {
        public string ReturnUrl
        {
            get
            {
                string url = SafeSC.GetRequest("r");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.Url.AbsolutePath.ToLower().EndsWith(".aspx")) { Response.Redirect("Login"); }
                //显示申请功能
            ...
            {
                AdminP_T.Attributes.Add("onfocus", "ShowSoftKeyboard(this)");
            }
            if (getVcount < 3)
            {
                VCode.Visible = false;
        ...
        {
            M_AdminInfo info = B_Admin.AuthenticateAdmin(AdminN_T.Text, AdminP_T.Text);
            if (info == null || info.IsNull)
            {
                getVcount = getVcount + 1;
                function.WriteErrMsg("用户名或密码错误！", "Login");
            }
            else
            {
                badmin.SetLoginState(info);
                if (string.IsNullOrEmpty(ReturnUrl)) { Response.Redirect(CustomerPageAction.customPath2 + "Default.aspx"); }
