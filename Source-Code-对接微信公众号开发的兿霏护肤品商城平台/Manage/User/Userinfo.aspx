...
<tr>
	...
	<td colspan="3">
		...
		<a href="UserModify.aspx?UserID=<%=mu.UserID %>" class="btn btn-default margin_l5" title="修改信息"><i class="fa fa-pencil"></i>修改</a>
		<%if (!pmu.IsNull)
    ...
    { %><button type="button" class="btn btn-default">无推荐人</button>
        <%} %>
	</td>
</tr>
<tr>
	...
	<td colspan="3">
		<a href="UserExp.aspx?UserID=<%=mu.UserID %>&type=1" class="btn btn-default" title="管理资金">资金：<%=mu.Purse %></a>
		<a href="UserExp.aspx?UserID=<%=mu.UserID%>&type=2" class="btn btn-default" title="管理银币">银币：<%=mu.SilverCoin %></a>
		<a href="UserExp.aspx?UserID=<%=mu.UserID %>&type=3" class="btn btn-default" title="管理积分">积分：<%=mu.UserExp %></a>
		<a href="UserExp.aspx?UserID=<%=mu.UserID %>&type=4" class="btn btn-default" title="管理点卷">点卷：<%=mu.UserPoint %></a>
		<a href="UserExp.aspx?UserID=<%=mu.UserID %>&type=5" class="btn btn-default" title="管理虚拟币">虚拟币：<%=mu.DummyPurse %></a>
		<a href="UserExp.aspx?UserID=<%=mu.UserID %>&type=6" class="btn btn-default" title="管理信誉值">信誉值：<%=mu.UserCreit %></a>
	</td>
</tr>
<tr>
	...
	<td colspan="3">
		<div class="input-group suinfo">
			...
			<span class="input-group-addon">推荐码</span>
			<label class="form-control">
				<%:GetPCode(mu.RoomID,mu.UserID) %>
			</label>
		</div>
	</td>
</tr>
<tr>
	<td class="text-right">绑定平台：</td>
	<td colspan="3">
		<div class="input-group pull-left" style="width: 240px;">
			<span class="input-group-addon"><i class="fa fa-wechat" style="color: green;"></i></span>
			<asp:Label runat="server" ID="Wechat_Info_T" CssClass="form-control disabled" Text="未绑定"></asp:Label>
			<span class="input-group-btn">
				<asp:Button runat="server" ID="Wechat_UnBind_Btn" class="btn btn-info" disabled="disabled" CommandArgument="wechat" Text="解除绑定" OnClick="Plat_UnBind_Btn_Click" OnClientClick="return confirm('确定要解除绑定吗,相关信息将会移除!');" />
			</span>
		</div>
		<div class="input-group  pull-left margin_l5" style="width: 240px;">
			<span class="input-group-addon"><i class="fa fa-weibo" style="color: red;"></i></span>
		...
		<div class="input-group  pull-left margin_l5" style="width: 240px;">
			<span class="input-group-addon"><i class="fa fa-qq" style="color: #5d5da5;"></i></span>
