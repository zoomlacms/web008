        ...
        try
        {
            ...
            switch (Action)
            {
                ...
                case "content_node":
                    {
                        M_CommonData conMod = conBll.SelReturnModel(Convert.ToInt32(Req("id")));
                        Response.Redirect(CustomerPageAction.customPath2 + "/Content/EditNode.aspx?NodeID=" + conMod.NodeID);
                    }
                    break;
                case "node_edit":
                    Response.Redirect(CustomerPageAction.customPath2 + "/Content/EditNode.aspx?NodeID=" + Req("id"));
                    break;
                case "node_tlp":
                    {
                    ...
                    break;
                case "node_list":
                    Response.Redirect(CustomerPageAction.customPath2 + "Content/NodeManage.aspx");
                    break;
                case "product_edit":
                ...
                case "product_node":
                    {
                        M_Product proMod = proBll.GetproductByid(Convert.ToInt32(Req("ID")));
                        Response.Redirect(CustomerPageAction.customPath2 + "/Content/EditNode.aspx?NodeID=" + proMod.Nodeid);
                    }
                    break;
                #endregion
                default:
